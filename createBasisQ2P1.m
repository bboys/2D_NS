...
% pressure, localPMass x_r(i,j) = int_0^1 int_0^1 basisP(j) d/dx_r basisF(i)
basisP(2) = x - 1/2;
basisP(3) = y - 1/2;
basisP(1) = 1;

localPMass.x = zeros(nrPBasisF,nrVBasisF);
localPMass.y = zeros(nrPBasisF,nrVBasisF);
for j = 1:nrVBasisF
    for i = 1:nrPBasisF
        localPMass.x(i,j) = int(int(basisFdx(j)*basisP(i),x,0,1),y,0,1);
        localPMass.y(i,j) = int(int(basisFdy(j)*basisP(i),x,0,1),y,0,1);
    end
end
localMatrix = struct('mass',localMass,'stiff', localStiff,...
     'pressure',localPMass, 'nonlin',localNonLin);
end
